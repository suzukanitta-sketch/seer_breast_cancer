/*I wanted to know why we only had data 2004-2009 in the analysis, 
I fixed it. We only have metastic cancer variables that go back to 1988
so we only have yrs of diagnosis 1988-2009 but I think that's okay.
We still have 58654 observations.
However, if we include hormone receptivity, races, etc in our analysis,
we have less data because I guess that variable is missing
for some people and SAS just deletes those.
So right now, I want to figure out which variables out of 
<age, race/ethnicity, year of diagnosis, ER/PR status, cause of death, 
CS site specific factor, CS Mets at diagnosed sites, and regional codes>
would further limit our data.
And I wanna do a logistic regression (I was trying at the very bottom)
...
/*without the metastic restriction, 1624459 observations,year_dx is from 1973*/
run;                    
...
if survtime=9999 then delete;    
/*with the metastic restriction, 58654 observations, year_dx is from 1988*/    
run;        


data cancer.logregression1;
set cancer.cohort;
length survtime 4;
survtime=srv_time_mon; 
if survtime>=0060 then do; long=1; end;
if survtime<0060 then do; long=0; end;
run;

proc logistic data=cancer.logregression1;
class year_dx;
model long = year_dx;
run;
